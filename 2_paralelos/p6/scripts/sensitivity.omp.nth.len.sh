...
#: ${CPU:?"Hay que inicializar la variable CPU (source ./init_cpuname.sh)"}
    ...
    for function in "${function_list[@]}"; do
        ...
        for precision in "${precision_list[@]}"; do
            ...
            for metric in "${metric_list[@]}"; do
                outfile=${directory}/data/sensitivity.nth.len.${loop}.${function}.${precision}.${compiler}.txt
                mkdir -p "${outfile%/*}"
                # sort -V: natural sort of numbers within text
                # set -x
                # grep --with-filename -w "${loop}" ${directory}/${loop}.${function}.*.${precision}.${compiler}* | sort -V | awk -v var="${metric}" '{ print $var }'
                grep --with-filename -w "${loop}" ${directory}/${loop}.${function}.*.${precision}.${compiler}* | sort -V | awk -v var="${metric}" '{ print $var }' > ${outfile}
                # set +x
            done
        done
    done
