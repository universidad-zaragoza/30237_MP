...
RGB2YCbCr_cast0(image_t * restrict image_in, image_t * restrict image_out)
{
    ...
    end_t = get_wall_time();
    results(end_t - start_t, height*width, "RGB2YCbCr_cast0");
}
//----------------------------------------------------------------------------
...
RGB2YCbCr_SOA1(image_t * restrict image_in, image_t * restrict image_out)
{
    ...
    end_t = get_wall_time();
    results(end_t - start_t, height*width, "RGB2YCbCr_SOA1");
...
YCbCr2RGB_conversion(image_t * image_in, image_t * image_out)
{
    ...
    for (int it = 0; it < NITER; it++)
    {
        ...
        {
            pixels_out[3*i + 0] = (unsigned char)
               (YCbCr2RGB[0][0]*(pixels_in[3*i + 0] - YCbCr2RGB_offset[0][0]) + 
                YCbCr2RGB[0][1]*(pixels_in[3*i + 1] - YCbCr2RGB_offset[0][1]) + 
                YCbCr2RGB[0][2]*(pixels_in[3*i + 2] - YCbCr2RGB_offset[0][2]));
            pixels_out[3*i + 1] = (unsigned char)
               (YCbCr2RGB[1][0]*(pixels_in[3*i + 0] - YCbCr2RGB_offset[1][0]) + 
                YCbCr2RGB[1][1]*(pixels_in[3*i + 1] - YCbCr2RGB_offset[1][1]) + 
                YCbCr2RGB[1][2]*(pixels_in[3*i + 2] - YCbCr2RGB_offset[1][2]));
            pixels_out[3*i + 2] = (unsigned char)
               (YCbCr2RGB[2][0]*(pixels_in[3*i + 0] - YCbCr2RGB_offset[2][0]) + 
                YCbCr2RGB[2][1]*(pixels_in[3*i + 1] - YCbCr2RGB_offset[2][1]) + 
                YCbCr2RGB[2][2]*(pixels_in[3*i + 2] - YCbCr2RGB_offset[2][2]));
        }
        dummy(image_in, image_out);
    }
    end_t = get_wall_time();
