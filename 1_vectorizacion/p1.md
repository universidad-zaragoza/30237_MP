...
7.  **Optativo**.
    ...
    $$
    \begin{aligned}
        ICOUNT = N \cdot LEN \cdot NTIMES = 6 \cdot 1024 \cdot 15 \cdot 1024^2 = 96.64 \cdot 10^9
    \end{aligned}
    $$        

    siendo $N$ el número de instrucciones del cuerpo del bucle (6), $LEN$ las iteraciones
    del bucle interno (1024) y $NTIMES$ las iteraciones del bucle externo ($15 \cdot 1024^2$).

    El número de ejecuciones del cuerpo del bucle interno (`EXECUTIONS`) es 16106127360,
    que coincide con el producto $LEN \cdot NTIMES = 1024 \cdot 15 \cdot 1024^2$.
