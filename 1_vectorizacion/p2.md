...
4.  Observa en el siguiente enlace el ensamblador del bucle en `ss_align_v2()` generado por las versiones 7.2 y 12.2 de `gcc`:
    
    [https://godbolt.org/z/heWTcbbWh](https://godbolt.org/z/heWTcbbWh)    
...
5.  Ejecuta las distintas funciones del programa `ss_align`:
...
La función `ss_alias_v1()` calcula el kernel _scale and shift_ ($y = alpha \cdot x + beta$).
...
1.  Compila con `gcc` el programa `ss_alias.c`:
    ...
    Observa el informe del compilador. ¿Ha vectorizado el bucle en `ss_alias_v1()`?  
...
el kernel _scale and shift_ para **uno de cada dos elementos**:
...
2.  Compila con `icx` el programa `ss_stride.c`:

            $ ./comp.sh -f ss_stride.c -c icx
