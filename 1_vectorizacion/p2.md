...
La función `axpby_align_v1()` ejecuta el kernel AXPBY.
...
3.  Las funciones `axpby_align_v1_intr()` y `axpby_align_v2_intr()` implementan
    ...
    Nota: en generaciones de procesadores Intel anteriores a Haswell,
...
4.  Observa en el siguiente enlace el ensamblador del bucle en `axpby_align_v2()` generado por las versiones 7.2 y 13.2 de `gcc`:
    
    [https://godbolt.org/z/b3hWGxTvq](https://godbolt.org/z/b3hWGxTvq)    
...
6.  La función `axpby_align_v1_intru()` es igual que `axpby_align_v1_intr()` excepto en que
...
2.  La función `axpby_alias_v2()` es una versión de `axpby_alias_v1()` en la que
...
el kernel AXPBY para **uno de cada dos elementos**:
...
2.  Compila con `icx` el programa `axpby_stride.c`:

            $ ./comp.sh -f axpby_stride.c -c icx
