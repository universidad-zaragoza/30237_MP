...
3.  Las funciones `axpy_align_v1_intr()` y `axpy_align_v2_intr()` implementan
    ...
    Observa el ensamblador del bucle en `axpy_align_v2()` y compáralo con el generado en el apartado anterior.
    
4.  Ejecuta las dos versiones del programa `axpy_align`:
...
5.  La función `axpy_align_v1_intru()` es igual que `axpy_align_v1_intr()` excepto en que
    ...
    carga en `gdb` el binario y el fichero `core` generado:
            ...
            $ gdb axpy_align.1k.single.native.gcc core.xyz
    ...
    vuelve a ejecutar:
            ...
            $ ulimit -c unlimited
            $ ./axpy_align.1k.single.native.gcc
